    ...
    <article id={project.slug} className="group card overflow-hidden">
      <Link href={project.href} className="block">
        <div className="relative aspect-[16/11] overflow-hidden bg-[var(--surface)]">
          ...
          />
          <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(8,8,8,0.1),rgba(8,8,8,0.28)_45%,rgba(8,8,8,0.74))]" />
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
            <div className="flex flex-wrap gap-3 text-[11px] uppercase tracking-[0.28em] text-white/72">
              <span>{project.location}</span>
              <span>{project.year}</span>
              <span>{project.status}</span>
            </div>
            <h3 className="mt-3 max-w-xl text-2xl sm:text-[2rem]">{project.title}</h3>
          </div>
        </div>
